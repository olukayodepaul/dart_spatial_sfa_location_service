...
from app.di.dependencies import continent_service, country_service, state_service
...
@router.get("/state/{state_id}", status_code=status.HTTP_200_OK)
async def get_all_state(state_id: int, is_state: State_Service = Depends(state_service), add_route_protection = Depends(get_current_user)):
    return is_state.get_state(state_id)
